...
@interface HelpPlayView ()

@end

@implementation HelpPlayView
...
- (void)didChangeContainerViewOffset:(CGPoint)offset withContainerSize:(CGSize)containerSize
{
    ...
    if (self.frame.size.width < offset.x) {
        positionX = self.imageView.layer.position.x - offset.x + self.frame.size.width;
    } else {
        positionX = self.imageView.layer.position.x + self.frame.size.width - offset.x;
    }
